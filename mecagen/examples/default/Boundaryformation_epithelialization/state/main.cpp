...
using namespace mg;
  ...
  int main( int argc, const char* argv[] ){
    ...
    load< MetaParam<HOST> >(metaParam, "../../BoxTissueFrom1cell_grn_default/output1682cells/metaparam_archive.xml");
    ...
    load< State_Host >(state, "../../BoxTissueFrom1cell_grn_default/output1682cells/state_archive.xml");
    ...
    for(int i=0; i<state.numCells[0]; i++){
      ...
      for(int j=0; j < NUMPROTEINmax; j++){
        if(j==3){
          state.cellProtein[i*NUMPROTEINmax+j] = 66.667;
        }
    ...
    for(uint i=0; i<NUMCELLmax; i++){
      state.cellRadius[i].y = 15.0;
    }  
